...
if [ -z "$bank" ] && [ -z "$YAMEM_ALLOW_FULL_INDEX" ] &&
   ! printf '%s' "$GIT_INDEX_FILE" | grep -q 'next-index'; then
    ...
    if [ -z "$inflight" ]; then
        ...
        if [ "$live" -gt 1 ]; then
            # ⚠️ Без согласования числительного: «работают 5 сессии» ловится глазом читателя
            # раньше, чем смысл сообщения (замечание Яна, 25.08).
            echo "yamem: живых сессий в памяти — $live; коммит без путей заберёт чужое из общего индекса."
...
if [ -f "$scripts/check-control-chars.py" ]; then
    PYTHONIOENCODING=utf-8 "$python" "$scripts/check-control-chars.py" || rc=1
fi
